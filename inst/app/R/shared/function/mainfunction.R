...
#' \item \code{"woa"} - Optimization using whale optimization algorithm based on Mirjalili and Lewis (2016).
#' }
...
#' ## tune the WOA parameter
#' woa_param <- c(vi.dist='normal',npar=10,same=15,woa.b=1)
#' ## FGWC with WOA optimization algorithm
#' res3 <- fgwc(census2010,census2010pop,census2010dist,'woa',param_fgwc,woa_param)
#' @export
...
fgwc <- function(data,pop,distmat,algorithm='classic',fgwc_param,opt_param){
  ...
  if (algorithm=='classic'){
    ...
    return(gwofgwc(data, pop, distmat, ncluster=ncluster, m=m, distance=distance, order=order,
                   alpha=alpha, a=a, b=b, error=error, max.iter=max.iter, randomN=randomN,
                   vi.dist=vi.dist, nwolf=npar, wolf.same=same))
  }
    ...
    opt_param <- get_param_woa(opt_param)
    return(woafgwc(data, pop, distmat, ncluster=ncluster, m=m, distance=distance, order=order,
                   alpha=alpha, a=a, b=b, error=error, max.iter=max.iter, randomN=randomN,
                   vi.dist=vi.dist, nwhale=npar,
                   woa.b=as.numeric(opt_param['woa.b']),
                   woa.same=same))
  }
...
get_param_gwo <- function(param){
  # GWO tidak memiliki parameter khusus di luar parameter universal
  # (npar, same, vi.dist sudah ditangani di fungsi fgwc() utama)
  return(c())
}
...
get_param_woa <- function(param){
  paramx <- c()
  if(is.na(param['woa.b'])|as.numeric(param['woa.b'])<=0) paramx['woa.b'] <- 1 else paramx['woa.b'] <- param['woa.b']
  return(paramx)
}