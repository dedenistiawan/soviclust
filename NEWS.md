...
# soviclust 0.7.0.9000

Development version. These changes are intended to form the basis of the `soviclust 0.8.0` release after NFE-fair optimizer validation and release-candidate checks are completed.

## Patch v3.1b — Exact NFE Budget Control

### Added

- Added optional `max_nfe` total Number of Function Evaluations (NFE) budget to all nine optimizers: ABC, FPA, GSA, GWO, HHO, IFA, PSO, TLBO, and WOA.
- Added exact total-NFE budget control covering both initialization and optimization evaluations.
- Added centralized budget checks before candidate evaluation to prevent NFE overshoot.
- Added budget-aware candidate evaluation so optimization can terminate cleanly when the evaluation budget is exhausted.
- Added result metadata:
  - `max_nfe`
  - `budget_exhausted`
  - `nfe_remaining`
  - `termination_reason`
- Added explicit termination reasons: `max_nfe`, `max_iter`, and `convergence`.
- Added regression tests for invalid NFE budgets, exact budget enforcement, no-overshoot behavior, reproducibility, backward compatibility, and controlled failure when the initialization budget is insufficient.

### Changed

- Extended all nine optimizer interfaces with `max_nfe = NULL` as the final argument to preserve compatibility with existing positional calls.
- Defined `max_nfe` as the total candidate-evaluation budget:

  `nfe_initialization + nfe_optimization <= max_nfe`

- Enforced NFE budget control at candidate-evaluation level rather than iteration level.
- Preserved feasible spatial Xie-Beni fitness, raw optimizer search state, spatial projection, and canonical optimizer movement formulas from Patch v3.
- Preserved Patch v3.1a behavior when `max_nfe = NULL`.

### Fixed

- Prevented optimizer runs from exceeding a user-specified total NFE budget.
- Added controlled termination when the budget is exhausted during optimization.
- Added controlled failure when `max_nfe` is too small to construct a valid initial population.
- Restored the canonical top-level `.soviclust_v3_new_position()` helper after the Patch v3.1b patch-application repair.

### Validation

- `devtools::test()` completed with 630 passing tests, 0 failures, 0 warnings, and 0 skipped tests.
- All Patch v3, Patch v3.1a, and Patch v3.1b regression tests passed.
- All nine optimizers preserved deterministic behavior under fixed random seeds.
- Exact-budget tests confirmed that total NFE does not exceed `max_nfe`.
- Backward-compatibility tests confirmed unchanged behavior when `max_nfe = NULL`.
- `R CMD check` completed with 0 errors and 0 warnings; the changelog heading structure was normalized here to remove the remaining NEWS version-parsing note.

## Patch v3.1a — NFE Instrumentation
...
## Patch v3 — Canonical Optimizer and Spatial Fitness Architecture
...
Release date: Historical package milestone (untagged)
...
Release date: 2024-07-01 - Pre-release
